...
if [[ $EP_SIZE -gt 1 ]]; then
    ...
    if [[ $GATE_VIEW_NUM -gt 1 ]]; then
        NAME="${NAME}-view-${GATE_VIEW_NUM}"
    fi
    
    if [[ $NO_MOE_LAYER_NUM -gt 1 ]]; then
        NAME="Relu-Threshold-local-${THRESHOLD}-${NAME}-ep-${EP_SIZE}-size-${MOE_FFN_HIDDEN_SIZE}-epConfig-skip${NO_MOE_LAYER_NUM}-${EP_INTERVAL}per-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-${FAKE_MOE_EVAL_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}"
    else
        NAME="Relu-Threshold-local-${THRESHOLD}-${NAME}-ep-${EP_SIZE}-size-${MOE_FFN_HIDDEN_SIZE}-interval-${EP_INTERVAL}-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-${FAKE_MOE_EVAL_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}"
    fi

    # NAME="Top${TOPK}-local-${NAME}-ep-${EP_SIZE}-size-${MOE_FFN_HIDDEN_SIZE}-interval-${EP_INTERVAL}-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-${FAKE_MOE_EVAL_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}"
    # NAME="HashLayer-${NAME}-ep-${EP_SIZE}-size-${MOE_FFN_HIDDEN_SIZE}-interval-${EP_INTERVAL}-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-${FAKE_MOE_EVAL_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}"
    # NAME="BaseLayer-${NAME}-ep-${EP_SIZE}-size-${MOE_FFN_HIDDEN_SIZE}-interval-${EP_INTERVAL}-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-${FAKE_MOE_EVAL_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}"
fi
...
mkdir -p "${OUTPUT_BASEPATH}/wikitext_log/"
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    DATA_PATH=/data/yuanhang/pretrain_data/gpt2-wikitext103_text_document
fi
###############################################################################
data_options=" \
...
megatron_options=" \
        ...
        --no-moe-layer-num ${NO_MOE_LAYER_NUM} \
        --yyh-relu \
        --use-threshold \
        --sparse-mlp"
#       --placeholder-expert
...
mkdir -p "yyh_config/"
config_json="yyh_config/ds_config_gpt_${NAME}.json"
...
run_cmd="deepspeed --include localhost:0,1 --master_port 61500 ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} &> ${OUTPUT_BASEPATH}/wikitext_log/${NAME}_${host}_${current_time}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
